...
interface EventsCinematicCanvasProps {
  progress: number;
}

// Weight calculation helper for smooth entrance and exit animations
function getSceneWeight(progress: number, start: number, end: number, isFirst = false, isLast = false) {
  if (progress < start || progress > end) return 0;
  const fadeDist = 0.05;
  if (isFirst) {
    if (progress <= end - fadeDist) return 1;
    return Math.max(0, (end - progress) / fadeDist);
  }
  if (isLast) {
    if (progress >= start + fadeDist) return 1;
    return Math.max(0, (progress - start) / fadeDist);
  }
  const mid = (start + end) / 2;
  if (progress < mid) {
    return Math.min(1, Math.max(0, (progress - start) / fadeDist));
  } else {
    return Math.min(1, Math.max(0, (end - progress) / fadeDist));
  }
}

// 1. 01 ROBOZAR (0.00 - 0.18)
function RoboSceneObject({ progress }: { progress: number }) {
  ...
  const weight = getSceneWeight(progress, 0.0, 0.18, true, false);
  ...
  const characterGroup = useMemo(() => {
    ...
    const targetHeight = isMobile ? 0.95 : 1.5;
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current) {
      const isVis = weight > 0.01;
      groupRef.current.visible = isVis;
      if (isVis) {
        const floatY = Math.sin(t * 1.3) * 0.04;
        const exitOffset = (1 - weight) * 0.6;
        const targetX = isMobile ? 0 : 1.25;
        const targetY = (isMobile ? 0.85 : 0) + floatY + exitOffset;
  ...
  return (
    <group ref={groupRef}>
      ...
      <pointLight position={[1.5, 1.5, 2.0]} intensity={3.5 * weight} color="#ffffff" />
      <pointLight position={[-1.2, -0.5, -1.0]} intensity={2.0 * weight} color="#38bdf8" />
    </group>
  );
}

// 2. 02 PLEXUS (0.16 - 0.36)
function PlexusSceneObject({ progress }: { progress: number }) {
  const groupRef = useRef<THREE.Group>(null);
  ...
  const weight = getSceneWeight(progress, 0.16, 0.36);
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current) {
      ...
      if (isVis) {
        const floatY = Math.sin(t * 1.3) * 0.04;
        const yOffset = (progress < 0.26 ? (1 - weight) * -0.5 : (1 - weight) * 0.5);
        const targetX = isMobile ? 0 : 1.25;
        const targetY = (isMobile ? 0.85 : 0) + floatY + yOffset;
        groupRef.current.position.set(targetX, targetY, 0);
        groupRef.current.scale.setScalar((isMobile ? 0.72 : 1.25) * Math.max(0.001, weight));
...
// 3. 03 KARYARACHNA (0.34 - 0.54)
function KaryarachnaSceneObject({ progress }: { progress: number }) {
  const groupRef = useRef<THREE.Group>(null);
  ...
  const weight = getSceneWeight(progress, 0.34, 0.54);
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current) {
      ...
      if (isVis) {
        ...
        const yOffset = (progress < 0.44 ? (1 - weight) * -0.5 : (1 - weight) * 0.5);
        const targetX = isMobile ? 0 : 1.25;
        const targetY = (isMobile ? 0.85 : 0) + floatY + yOffset;
        groupRef.current.position.set(targetX, targetY, 0);
        groupRef.current.scale.setScalar((isMobile ? 0.72 : 1.25) * Math.max(0.001, weight));
...
// 4. 04 KERMIS (0.52 - 0.72)
function KermisSceneObject({ progress }: { progress: number }) {
  const groupRef = useRef<THREE.Group>(null);
  ...
  const weight = getSceneWeight(progress, 0.52, 0.72);
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current) {
      ...
      if (isVis) {
        const floatY = Math.sin(t * 1.3) * 0.04;
        const yOffset = (progress < 0.62 ? (1 - weight) * -0.5 : (1 - weight) * 0.5);
        const targetX = isMobile ? 0 : 1.25;
        const targetY = (isMobile ? 0.85 : 0) + floatY + yOffset;
        groupRef.current.position.set(targetX, targetY, 0);
        groupRef.current.scale.setScalar((isMobile ? 0.72 : 1.25) * Math.max(0.001, weight));
...
// 5. 05 ELECTRICA (0.70 - 0.90)
function ElectricaSceneObject({ progress }: { progress: number }) {
  const groupRef = useRef<THREE.Group>(null);
  const coilRef = useRef<THREE.Group>(null);
  const { viewport } = useThree();
  const isMobile = viewport.width < 5.5;

  const weight = getSceneWeight(progress, 0.70, 0.90);
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current) {
      ...
      groupRef.current.visible = isVis;
      if (isVis) {
        const floatY = Math.sin(t * 1.0) * 0.04;
        const yOffset = (progress < 0.80 ? (1 - weight) * -0.5 : (1 - weight) * 0.5);
        const targetX = isMobile ? 0 : 1.25;
        const targetY = (isMobile ? 0.85 : 0) + floatY + yOffset;
        groupRef.current.position.set(targetX, targetY, 0);
        groupRef.current.scale.setScalar((isMobile ? 0.72 : 1.25) * Math.max(0.001, weight));
...
// 6. 06 MECHANICA (0.86 - 1.00)
function MechanicaSceneObject({ progress }: { progress: number }) {
  const groupRef = useRef<THREE.Group>(null);
  const coreRef = useRef<THREE.Mesh>(null);
  ...
  const weight = getSceneWeight(progress, 0.86, 1.00, false, true);
...
export default function EventsCinematicCanvas({ progress }: EventsCinematicCanvasProps) {
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none select-none z-10">
      <ErrorBoundary fallback={<div className="absolute inset-0 bg-transparent" />}>
        ...
        >
          ...
          <RoboSceneObject progress={progress} />
          <PlexusSceneObject progress={progress} />
          <KaryarachnaSceneObject progress={progress} />
          <KermisSceneObject progress={progress} />
          <ElectricaSceneObject progress={progress} />
          <MechanicaSceneObject progress={progress} />
        </Canvas>
      </ErrorBoundary>
    </div>
  );
