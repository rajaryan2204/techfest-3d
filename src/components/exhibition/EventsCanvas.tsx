...
// 1. 01 ROBOZAR (activeIdx === 0)
function RoboCharacterObject({ active }: { active: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current) {
      ...
      if (active) {
        ...
        const targetY = (isMobile ? 0.75 : 0) + floatY;
        groupRef.current.position.set(targetX, targetY, 0);

        const mouseX = !isMobile ? state.pointer.x * 0.12 : 0;
  ...
  return (
    <group ref={groupRef} visible={active}>
      ...
      <pointLight position={[1.5, 1.5, 2.0]} intensity={3.5} color="#ffffff" />
      <pointLight position={[-1.2, -0.5, -1.0]} intensity={2.0} color="#38bdf8" />
    </group>
  );
}

// 2. 02 PLEXUS Geometric Neural Lattice (activeIdx === 1)
function PlexusObject({ active }: { active: boolean }) {
  const groupRef = useRef<THREE.Group>(null);
  ...
  return (
    <group ref={groupRef} visible={active} scale={isMobile ? 0.85 : 1.25}>
...
// 3. 03 KARYARACHNA Emerald Polyhedron (activeIdx === 2)
function KaryarachnaObject({ active }: { active: boolean }) {
  const groupRef = useRef<THREE.Group>(null);
...
// 4. 04 KERMIS Amethyst Gyroscopic Rings (activeIdx === 3)
function KermisObject({ active }: { active: boolean }) {
  const groupRef = useRef<THREE.Group>(null);
...
// 5. 05 ELECTRICA Hyper-Bright Induction Coils (activeIdx === 4)
function ElectricaObject({ active }: { active: boolean }) {
  const groupRef = useRef<THREE.Group>(null);
...
// 6. 06 MECHANICA Polished Gear Transmission (activeIdx === 5)
function MechanicaObject({ active }: { active: boolean }) {
  const groupRef = useRef<THREE.Group>(null);
  const coreRef = useRef<THREE.Mesh>(null);
  ...
  return (
    <group ref={groupRef} visible={active} scale={isMobile ? 0.85 : 1.25}>
      ...
      {/* Outer Golden Transmission Ring */}
...
export default function EventsCanvas({ activeIdx }: EventsCanvasProps) {
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none select-none z-10">
      <ErrorBoundary fallback={<div className="absolute inset-0 bg-transparent" />}>
        ...
        >
          ...
          <RoboCharacterObject active={activeIdx === 0} />
          <PlexusObject active={activeIdx === 1} />
          <KaryarachnaObject active={activeIdx === 2} />
          <KermisObject active={activeIdx === 3} />
          <ElectricaObject active={activeIdx === 4} />
          <MechanicaObject active={activeIdx === 5} />
        </Canvas>
      </ErrorBoundary>
    </div>
  );
